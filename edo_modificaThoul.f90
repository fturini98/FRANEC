!**************************************************         
! MODIFICA DELLA ROUTINE DI THOUL & CO
! DA ME ESEGUITA
!**************************************************            
subroutine diffusionrad(M,A,Z,X,CL,grad_el,AP,AT,AX,AR)
  use interfaccia
  ...
  integer :: M
  real,dimension(M) :: A,Z,X,AP,AT,AR
  real,dimension(M-1) :: grad_el
  ...
  real,dimension(NMAX) :: gammatmp, ALPHA, NU, GA,mass, VR
  real :: C(MMAX),CC,AC 
  real,dimension(MMAX,MMAX) :: K
  real,dimension(NMAX,NMAX) :: GAMMA, DELTA
  real :: TEMP,KO,D, VARTMP 
  ...
  ! assegnazione del vettore VR(i)=alfa(i)*m(i)*g_rad(i)
  do j=1,N
     if (j .le. (M-1)) then
        mass(j)=1.6726d-24*A(J)
        VR(j)=alpha(j)*mass(j)*grad_el(j)
     else
        VR(j)=0.
     endif
  enddo
  ...
  call LUBKSB(DELTA,N,NMAX,INDX,VR) 
  do J=1,N 
     do I=1,N 
        GA(I) = GAMMA(I,J) 
     enddo
  ...
  do I=1,M 
     AP(I) = ALPHA(I) 
     AT(I) = NU(I) 
     AR(I) = -VR(I)*VARTMP*6.9599d3  ! raggio_solare/T_0 dove T_0=10^7 K, per adimensionalizzare correttamente
!!$     AR(I) = -VR(I)*VARTMP*6.9599d3*10. ! provo a moltiplicare per tot: vediamo che effetto fa
     do J=1,M 
        AX(I,J) = GAMMA(I,J) 
     end do
  end do
