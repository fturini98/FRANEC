...
subroutine convergenza_epsi_DM(epsi_DM_tot,T_DM,modello,tempo)
    ...
    call epsi_DM_routine(T_min,epsi_tot_min)

    if ((epsi_tot_min*epsi_DM_tot)>0) then
        write(*,*)"Moio, L'epsi max è", epsi_DM_tot,"l'epsi min è:", epsi_tot_min
        stop
        endif
    ...
    do i=1,max_cicli
        ...
        if (Rapporto_Lumi_DM>=errore_max .and. i==max_cicli) then
            ...
            write(ioDrakError,'(I0, A, ES25.15, A, ES25.15, A, ES25.15, A, ES25.15, A, ES25.15)')Modello,char(9),Tempo,char(9),T_DM,char(9),epsi_DM_tot,char(9),&
...
subroutine stampa_epsi_DM(modello,epsi_DM_tot,T_DM,Tempo)
...
!Il file relativo si chiama epsi_DM e la sua unit vale unit=ioDark==411(è definita all'interno del moduleo Dark_Matter)
    ...
    Rapporto_Lumi_DM=abs(epsi_DM_tot)*1e-32/&
                        (10.0**(ELLOG)*&
                        38.27)!Luminosità solare *10^-32 erg/s
    write(ioDark,'(I0, A, ES25.15, A, ES25.15, A, ES25.15, A, ES25.15, A, ES25.15, A, ES25.15)')Modello,char(9),Tempo,char(9),T_DM,char(9),epsi_DM_tot,char(9),10.0**(ELLOG)*38.27*1e32,char(9),Rapporto_Lumi_DM,char(9),10.0**TEFF
