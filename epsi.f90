subroutine EPSI (RO,T,JF,NABLA,HT,EPS,EPSA,ECAR,DELTA) 
  ...
  real :: xx1old, roan, tmp, rapp1, rapp2
  ...
  if(stdchim == 1) then
     if(T < 1.d5 .and. nabla /= 1) then 
        return 
     end if
     if(T < 1.d6 .and. NMD >= 40 .and. nabla /= 1) then 
        return 
     end if
     if(XX(1) <= 0. .and. XX(3) > 0. .and. T <= 4.d7  .and. nabla /= 1) then 
        return 
     end if
     if(XX(1) <= 0. .and. XX(3) <=  0. .and. T <= 4.d8 .and. nabla /= 1) then 
        return 
     end if
  endif
  ...
  if(IR <=  2) then 
     ...
     E(35) = Q(35)*S(35)*ROAN*YV(2)*YV(26)
  ...
  if(IR > 1) then
     E(7) = Q(28)*S(28)*RO**2*AVN*YV(3)**3/6.
...
!  if(EPSP > 0. .and. EPSC > 0.) then
  ! ripartisce energia da bruciamnto H tra pp e cno
  if(EPSP + EPSC > 0.) then
  ...
  Srho(nsezi+2) = RO * S(5) * rapp2

  KECUIL(JF) = 0
  if(G(6,1) < 0.0 .and. STDCHIM == 1 ) ISHEL = 1
  if(l3 > 0 .and. jf > l3) ishel = 1
  OISHEL = ISHEL
  ...
  if( STDCHIM == 1) then
     ...
     ! trattamento chimica con sistema eq. differenziali
     if(IR <= 2) then
        convettivo = vconv(jf)
        call check_convettivo(convettivo, jf, Ssum, yfs, Srho, yf, jfstart)

        ! sono alla fine di un blocco convettivo
        if (convettivo == 3 .or. (convettivo == 2 .and. jf == MAXME-1)) then
           yf = yfs
           
           ! calcolo la chimica di tutto il blocco convettivo
           call chimica(PAS, mele, YF, SSum)
           
           do mc=jfstart,jf
              do nel=3,9
                 xxx(nel,mc) = yf(nel)*at(nel)
              end do
              do nel=22,26
                 xxx(nel,mc) = yf(nel)*at(nel)
              end do
        ...
        else if(convettivo == 0 .or. convettivo == 4) then
           call chimica(PAS, mele, YF, Srho)
        endif
     else if(IR == 3) then
!        call chimica(PAS, mele, YF, Srho)
        call ELIO(RO,T,PAS,JF,INI,IFI,N,ISHORT, YV, YF, NEQUI, S)
